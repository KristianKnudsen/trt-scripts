...
def main(run_dir):
    ...
    result = {
        ...
        "bench_gpu_util_avg": gpu_util_avg,
        "bench_gpu_util_peak": gpu_util_peak,
    ...
    # ----------- ROUNDING ADDED HERE -----------
    def r1(x): return round(x, 1) if isinstance(x, (int, float)) and x is not None else x
    def r2(x): return round(x, 2) if isinstance(x, (int, float)) and x is not None else x
    def r0(x): return int(round(x)) if isinstance(x, (int, float)) and x is not None else x

    for k in [
        "latency_p50_ms", "latency_p90_ms", "latency_p95_ms", "latency_p99_ms",
        "token_latency_p50_ms", "token_latency_p90_ms", "token_latency_p95_ms", "token_latency_p99_ms",
        "ttft_p50_ms", "ttft_p90_ms", "ttft_p95_ms", "ttft_p99_ms"
    ]:
        result[k] = r1(result[k])

    for k in ["req_per_s", "tokens_per_s", "output_tokens_per_s"]:
        result[k] = r2(result[k])

    result["bench_vram_peak_mb"] = r0(result["bench_vram_peak_mb"])
    result["bench_power_avg_w"] = r0(result["bench_power_avg_w"])
    result["bench_power_peak_w"] = r0(result["bench_power_peak_w"])

    result["bench_gpu_util_avg"] = r1(result["bench_gpu_util_avg"])
    result["bench_gpu_util_peak"] = r1(result["bench_gpu_util_peak"])
    # -------------------------------------------

    print(json.dumps(result, indent=2))

        # Also print a one line header and value row for Excel
    ...
    print(sep.join(keys))
    print(sep.join(
        "" if result[k] is None else str(result[k])
        for k in keys
    ))
